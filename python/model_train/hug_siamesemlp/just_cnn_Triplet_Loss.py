...
class AdaptiveMargin:
    ...
    def get_margin(self):
        return max(0.1, self.neg_mean - self.pos_mean) # 避免margin为负
...
def train(train_root, test_root, num_epochs=100, patience_limit=10, best_acc=0.0,last_epoch=-1, warmup_epochs=10, num_cycles=2,decay_factor= 0.3):
    ...
    os.makedirs("model_pth/jus_cnn_se_Triplet_Loss_hard_normalize_L2", exist_ok=True)
    torch.save(best_model_wts, f"model_pth/jus_cnn_se_Triplet_Loss_hard_normalize_L2/best_model_best_acc_{best_acc:.4f}_best_f1_{best_f1:.4f}_best_thresh_{best_epoch}.pth")
    duration = 1500  # 毫秒
    freq = 440  # 频率 (Hz)
